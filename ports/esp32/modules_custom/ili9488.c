...
static int line_thickness = 1;  // Default line thickness
static uint8_t current_orientation = ORIENTATION_PORTRAIT;
static int display_width = ILI9488_PHYS_WIDTH;   // Logical width
static int display_height = ILI9488_PHYS_HEIGHT; // Logical height
...
static uint8_t get_madctl_for_orientation(uint8_t orientation) {
    uint8_t madctl = MADCTL_BGR;  // Base setting with BGR color order
    ...
    switch (orientation) {
        ...
        case ORIENTATION_LANDSCAPE_INV:
            // 270 degrees clockwise (90 counter-clockwise): 480x320
...
static mp_obj_t ili9488_init(size_t n_args, const mp_obj_t *args) {
    ...
    if (current_orientation == ORIENTATION_LANDSCAPE || 
        ...
        display_width = ILI9488_PHYS_HEIGHT;   // 480
        display_height = ILI9488_PHYS_WIDTH;   // 320
    } else {
        display_width = ILI9488_PHYS_WIDTH;    // 320
        display_height = ILI9488_PHYS_HEIGHT;  // 480
    }
    ...
    // Configure SPI with larger max_transfer_sz
    ...
    uint8_t pixfmt = 0x66; // 18-bit color (6-6-6 RGB)
    ili9488_write_data(&pixfmt, 1);

    // Configure MADCTL (Memory Access Control) with selected orientation
    ...
    // Allocate framebuffer in PSRAM (3 bytes per pixel for RGB888)
    // Framebuffer size is based on logical dimensions
    if (framebuffer == NULL) {
    ...
    mp_printf(&mp_plat_print, "ILI9488: Init complete\n");
...
static mp_obj_t ili9488_set_line_thickness(mp_obj_t thickness_obj) {
    ...
    if (thickness > 20) thickness = 20;  // Reasonable maximum
...
// Partial update - only update a rectangular region
static mp_obj_t ili9488_update_region(size_t n_args, const mp_obj_t *args) {
    if (!framebuffer || !spi_device) return mp_const_none;
    ...
    // Clamp to screen bounds (logical coordinates)
    if (x < 0) { w += x; x = 0; }
    if (y < 0) { h += y; y = 0; }
    ...
    // Send only the affected region
    for (int row = 0; row < h; row++) {
        int fb_offset = ((y + row) * display_width + x) * 3;
        ili9488_write_data((uint8_t *)framebuffer + fb_offset, w * 3);
    }
...
static mp_obj_t ili9488_line(size_t n_args, const mp_obj_t *args) {
    ...
    if (line_thickness == 1) {
        // Fast path for single pixel line
        int dx = abs(x1 - x0);
        int dy = abs(y1 - y0);
        int sx = x0 < x1 ? 1 : -1;
        int sy = y0 < y1 ? 1 : -1;
    ...
    // Thick line implementation with scanline fill
    int dx = x1 - x0;
    int dy = y1 - y0;
    float length = sqrt(dx * dx + dy * dy);
    
    if (length < 0.1f) {
        // Zero-length line - draw filled circle
    ...
    // Check if line is perfectly horizontal or vertical (fast paths)
    if (dy == 0) {
        // Horizontal line - very fast
    ...
    // Angled thick line with scanline fill
    ...
    // Calculate the 4 corners of the thick line rectangle
    ...
    // Add padding for rounding
    min_y -= 1;
    max_y += 1;
    
    // Scanline fill algorithm
    for (int scan_y = min_y; scan_y <= max_y; scan_y++) {
        // Find intersections with the 4 edges of the rectangle
        int intersections[4];
    ...
    // Add rounded endcaps for smooth appearance
    int radius = line_thickness / 2;
...
// Draw rectangle (outline only or filled)
static mp_obj_t ili9488_rect(size_t n_args, const mp_obj_t *args) {
    ...
    // Fill if fill_color is provided
...
// Fill rectangle (for backward compatibility)
static mp_obj_t ili9488_fill_rect(size_t n_args, const mp_obj_t *args) {
    int x = mp_obj_get_int(args[0]);
    int y = mp_obj_get_int(args[1]);
    int w = mp_obj_get_int(args[2]);
    int h = mp_obj_get_int(args[3]);
    ...
    for (int j = 0; j < h; j++) {
        for (int i = 0; i < w; i++) {
            set_pixel_fb(x + i, y + j, color);
        }
    }

    return mp_const_none;
}
static MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(ili9488_fill_rect_obj, 5, 5, ili9488_fill_rect);

// Draw circle (Midpoint circle algorithm, outline or filled)
...
// Fill circle (for backward compatibility)
static mp_obj_t ili9488_fill_circle(size_t n_args, const mp_obj_t *args) {
    int x0 = mp_obj_get_int(args[0]);
    int y0 = mp_obj_get_int(args[1]);
    int r = mp_obj_get_int(args[2]);
    uint32_t color = mp_obj_get_int(args[3]);
...
static MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(ili9488_fill_circle_obj, 4, 4, ili9488_fill_circle);
...
static mp_obj_t ili9488_triangle(size_t n_args, const mp_obj_t *args) {
    ...
    // Fill triangle if fill_color is provided
    if (fill_color != COLOR_NONE) {
...
// Sprite structure with dirty rectangle tracking
typedef struct {
    mp_obj_base_t base;
    uint8_t *pixels;      // RGB888 format (3 bytes per pixel)
    uint8_t *background;  // RGB888 format (3 bytes per pixel)
    int width;
    int height;
    int x;
    int y;
    int old_x;            // Previous position for dirty rect
    int old_y;
    bool visible;
    bool moved;           // Track if sprite moved
} sprite_obj_t;
...
// Draw sprite with dirty rectangle tracking
static mp_obj_t sprite_draw(size_t n_args, const mp_obj_t *args) {
    ...
    bool positions_overlap = false;
    
    // Check if old and new positions overlap
    if (sprite_moved) {
        int old_right = self->x + self->width;
        int old_bottom = self->y + self->height;
        int new_right = new_x + self->width;
        int new_bottom = new_y + self->height;
        
        positions_overlap = !(new_x >= old_right || new_right <= self->x ||
                             new_y >= old_bottom || new_bottom <= self->y);
    }

    // Step 1: If sprite was visible, restore its old background
    if (self->visible) {
    ...
    // Step 2: NOW save clean background at new position (sprite is erased)
    for (int j = 0; j < self->height; j++) {
        for (int i = 0; i < self->width; i++) {
            int px = new_x + i;
            int py = new_y + j;
    ...
    // Step 3: Draw sprite at new position (skip black/transparent pixels: 0x000000)
    for (int j = 0; j < self->height; j++) {
        for (int i = 0; i < self->width; i++) {
            int px = new_x + i;
            int py = new_y + j;
            ...
            // Skip if color is black (transparent)
            if ((r != 0 || g != 0 || b != 0) && 
    ...
    // Step 4: Update display if auto_update is enabled
    if (auto_update && spi_device) {
        if (sprite_moved) {
            ...
            if (update_w > 0 && update_h > 0) {
                ...
                for (int row = 0; row < update_h; row++) {
                    ...
                    ili9488_write_data((uint8_t *)framebuffer + fb_offset, update_w * 3);
                }
            }
        } else {
            // First draw or no movement - just update sprite region
            if (new_x >= 0 && new_y >= 0 && 
                ...
                for (int row = 0; row < self->height; row++) {
                    ...
                    ili9488_write_data((uint8_t *)framebuffer + fb_offset, self->width * 3);
                }
            }
        }
    }
...
static mp_obj_t ili9488_show(void) {
    ...
    // Use smaller chunk size to avoid SPI transaction limit (typically 4092 bytes)
    const size_t chunk_size = 4092;  // Just under 4KB to be safe
    size_t total_bytes = display_width * display_height * 3;

    mp_printf(&mp_plat_print, "ILI9488: Updating display (%d bytes)...\n", total_bytes);

    for (size_t offset = 0; offset < total_bytes; offset += chunk_size) {
        size_t len = (offset + chunk_size > total_bytes) ?
                     (total_bytes - offset) : chunk_size;
        ili9488_write_data((uint8_t *)framebuffer + offset, len);
    }

    mp_printf(&mp_plat_print, "ILI9488: Display updated\n");
...
static const mp_rom_map_elem_t ili9488_module_globals_table[] = {
    ...
    { MP_ROM_QSTR(MP_QSTR_fill_rect), MP_ROM_PTR(&ili9488_fill_rect_obj) },
    { MP_ROM_QSTR(MP_QSTR_circle), MP_ROM_PTR(&ili9488_circle_obj) },
    { MP_ROM_QSTR(MP_QSTR_fill_circle), MP_ROM_PTR(&ili9488_fill_circle_obj) },
